...
# Intentar importar meses desde descargar_archivos
try:
    from v1.descargar_archivos import meses
except ImportError:
    try:
        from descargar_archivos import meses
    except ImportError:
        # Definir meses localmente si no se puede importar
        meses = {
            1: "Enero",
            2: "Febrero",
            3: "Marzo",
...
def leer_excel_pandas(ruta_archivo: Union[str, Path], hoja: Optional[str] = None, 
...
def leer_celda_excel(ruta_archivo: Union[str, Path], hoja: str, celda: str) -> any:
    """
...
def leer_rango_excel(ruta_archivo: Union[str, Path], hoja: str, 
                     rango: str) -> pd.DataFrame:
    """
...
class LectorBalance:
    ...
    def __init__(self, anyo: int, mes: int, carpeta_base: str = "bd_data"):
        ...
        if self.ruta_archivo is None:
            ...
            raise FileNotFoundError(
                f"No se encontró el archivo Balance para {mes}/{anyo} en {carpeta_base}"
            )
    ...
    def leer_celda(self, hoja: str, celda: str) -> any:
        """
    ...
    def leer_balance_valorizado(self, header: Optional[int] = None) -> pd.DataFrame:
        ...
        if header is None:
            print(f"  Detectando automáticamente la fila de encabezados...")
            header = self._detectar_fila_encabezados(nombre_hoja)
            if header is None:
                print(f"[WARNING] No se pudo detectar automáticamente la fila de encabezados, usando fila 0")
                header = 0
            else:
        ...
        if "barra" not in columnas_lower:
            print(f"[WARNING] No se encontró la columna 'barra' en los encabezados")
            print(f"  Columnas disponibles: {', '.join([str(col) for col in df.columns[:10]])}...")
        if "monetario" not in columnas_lower:
            print(f"[WARNING] No se encontró la columna 'monetario' en los encabezados")
            print(f"  Columnas disponibles: {', '.join([str(col) for col in df.columns[:10]])}...")
    ...
    def buscar_por_barra(self, df: pd.DataFrame, nombre_barra: str) -> pd.DataFrame:
        ...
        if columna_barra is None:
            print(f"[ERROR] La columna 'barra' no se encontró en el DataFrame")
        ...
        if columna_monetario:
            ...
            print(f"[ERROR] La columna 'monetario' no se encontró")
        
        return df_filtrado
    
    def guardar_en_plantilla(self, df_balance: pd.DataFrame, ruta_plantilla: str = "plantilla_base.xlsx", 
                             nombre_barra: Optional[str] = None, nombre_empresa: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if not columna_barra or not columna_monetario:
                print(f"[ERROR] No se encontraron las columnas 'barra' o 'monetario'")
                return False
            ...
            if nombre_barra or nombre_empresa:
                ...
                if nombre_empresa:
                    if columna_empresa is None:
                        print(f"[ERROR] No se encontró la columna 'nombre_corto_empresa'")
                        print(f"  Columnas disponibles: {', '.join([str(c) for c in df_balance.columns[:20]])}...")
                        return False
                ...
                if len(df_guardar) == 0:
                    print(f"[WARNING] No se encontraron registros con los filtros especificados")
                ...
                print(f"  Agrupando por barra y sumando valores monetarios")
            ...
            print(f"[OK] Datos guardados exitosamente")
            print(f"  Total de filas escritas: {len(df_guardar)}")
            print(f"  Total de columnas escritas: {len(df_guardar.columns)}")
            if nombre_barra:
                print(f"  Todas las columnas de las filas filtradas fueron guardadas")
            ...
            if (nombre_barra or nombre_empresa) and len(df_guardar.columns) < len(df_balance.columns):
                print(f"[WARNING] Se guardaron {len(df_guardar.columns)} columnas, pero el DataFrame original tiene {len(df_balance.columns)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Ejemplo: leer la hoja "Balance Valorizado"
        print("\n" + "="*50)
        # Intentar leer con diferentes filas como encabezado
        df_balance = None
        for header_row in [0, 1, 2, 3]:
            try:
                df_balance = lector.leer_balance_valorizado(header=header_row)
                # Verificar si tiene la columna "monetario"
                columnas_lower = [str(col).lower() for col in df_balance.columns]
                if "monetario" in columnas_lower:
                    print(f"\n[OK] Encabezados encontrados en la fila {header_row}")
                    break
            except:
                continue
        
        if df_balance is None:
            df_balance = lector.leer_balance_valorizado(header=0)
        
        print(f"\nPrimeras filas del Balance Valorizado:")
        print(df_balance.head(10))
        print(f"\nColumnas disponibles:")
        for i, col in enumerate(df_balance.columns, 1):
            print(f"  {i}. {col}")
        
        # Guardar datos en plantilla (agrupados por barra)
        print("\n" + "="*50)
        print("Guardando datos en plantilla_base.xlsx")
        print("="*50)
        
        # Guardar todos los datos agrupados por barra
        exito = lector.guardar_en_plantilla(df_balance, ruta_plantilla="plantilla_base.xlsx")
        ...
        # Opcional: Búsqueda por barra específica
        print("\n" + "="*50)
        respuesta = input("¿Desea buscar valores para una barra específica? (s/n): ").strip().lower()
        
        if respuesta == 's':
            # Obtener lista de barras únicas disponibles
            columna_barra = None
            for col in df_balance.columns:
                if str(col).lower() == "barra":
                    columna_barra = col
            ...
            if columna_barra:
                barras_unicas = sorted(df_balance[columna_barra].dropna().unique())
                print(f"\nTotal de barras disponibles: {len(barras_unicas)}")
                print(f"\nPrimeras 20 barras disponibles:")
                for i, barra in enumerate(barras_unicas[:20], 1):
                    print(f"  {i}. {barra}")
                if len(barras_unicas) > 20:
                    print(f"  ... y {len(barras_unicas) - 20} más")
            
            # Solicitar entrada del usuario
            print("\n" + "-"*50)
            barra_usuario = input("Ingrese el nombre de la barra a buscar: ").strip()
            
            if barra_usuario:
                df_barra = lector.buscar_por_barra(df_balance, barra_usuario)
